...
class BrowserManager {
  ...
  useBrowserOptions({ authenticationMethod, ...config }) {
    this.config = config;
  ...
  async createNew({ for: user } = {}) {
    ...
    if (!this.authenticationMethod) {
      throw new Error('Can\'t log user in without providing an authenticationMethod.');
    }
    const browser = new Browser(unmanagedBrowser, this.config, this.authenticationMethod);
